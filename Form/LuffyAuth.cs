    ...
    public class api
    {
        ...
        public api(string name, string ownerid, string version, string path = null)
        {
            if (ownerid.Length != 10)
            {
                ...
                Process.Start("https://auth.luffycpp.in/app/");
                Thread.Sleep(2000);
        ...
        private static async Task<string> req(NameValueCollection post_data)
        {
            try
            {
                if (FileCheck("auth.luffycpp.in")) // change this url if you're using a custom domain
                {
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    HttpResponseMessage response = await client.PostAsync("https://auth.luffycpp.in/api/1.2/index.php", content); // change this url if you're using a custom domain
        ...
        {
            return; // Bypass signature check for self-hosted instances
